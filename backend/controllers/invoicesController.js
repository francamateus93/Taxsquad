...
export const updateInvoice = async (req, res) => {
  ...
  try {
    await db.execute(
      `UPDATE invoices SET number=?, date=?, client_name=?, client_id=?, client_address=?, city=?, country=?, concept=?, quantity=?, price=?, vat=?, irpf=?, currency=?, payment_method=?, total_amount=? WHERE id=?`,
      [
        number,
        date,
        client_name,
